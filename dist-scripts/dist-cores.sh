...
for f in *_${platform}.${EXT} ; do
   ...
   if [ $platform = "ps3" ] ; then
      ...
      COUNTER=COUNTER+1
      mkdir -p ../pkg/${platform}/${name}_libretro.vpk-folder/vpk/sce_sys/
      vita-make-fself ../retroarch_${platform}.velf ../pkg/${platform}/${name}_libretro.vpk-folder/vpk/eboot.bin
      vita-mksfoex -s TITLE_ID=RETR0000${COUNTER} "RetroArch ${name}" ../pkg/${platform}/${name}_libretro.vpk-folder/vpk/sce_sys/param.sfo
   elif [ $PLATFORM = "ctr" ] ; then
