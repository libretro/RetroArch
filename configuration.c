...
void config_set_defaults(void* data)
{
    ...
    *settings->paths.path_menu_xmb_font = '\0';
    ...
    if (g_defaults.settings_video_refresh_rate > 0.0 && g_defaults.settings_video_refresh_rate != DEFAULT_REFRESH_RATE)
    ...
    const char* path, settings_t* settings)
{
   ...
   if (verbosity_is_enabled())
   {
    ...
    if (frontend_driver_has_gamemode() && !frontend_driver_set_gamemode(settings->bools.gamemode_enable) && settings->bools.gamemode_enable) {
...
bool config_save_file(const char* path)
{
    ...
    msg_color = (((int)(settings->floats.video_msg_color_r * 255.0f) & 0xff) << 16) + (((int)(settings->floats.video_msg_color_g * 255.0f) & 0xff) << 8) + (((int)(settings->floats.video_msg_color_b * 255.0f) & 0xff));
...
void config_load_file_salamander(void)
{
    ...
    if (config_get_path(config, "libretro_path",
        ...
        && !string_is_empty(libretro_path) && !string_is_equal(libretro_path, "builtin"))
...
{
    const struct input_bind_map* keybind = (const struct input_bind_map*)INPUT_CONFIG_BIND_MAP_GET(bind_index);
    if (!keybind)
        return false;
    ...
    const char* axis, void* bind_data)
{
    ...
    if (tmp_a && (!string_is_empty(tmp_a->value))) {
        if (bind->joyaxis_label && !string_is_empty(bind->joyaxis_label))
