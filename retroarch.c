...
static void merge_input_bits(input_bits_t *target, input_bits_t *src)
{
   unsigned i;
   uint8_t *p_src = (uint8_t *)src;
   uint8_t *p_target = (uint8_t *)target;

   for(i = 0; i < sizeof(input_bits_t); i++) {
      p_target[i] |= p_src[i];
   }
}

/* NOTE: menu_input_active is always false if HAVE_MENU is not set */
static void collect_system_input(struct rarch_state *p_rarch, settings_t *settings, input_bits_t *current_bits)
{
   unsigned port;
   ...
   bool menu_input_active              = menu_state_get_ptr()->alive && !(settings->bools.menu_unified_controls && !display_kb);
#else
               ...
               &&  current_input->input_state)
         {
            ...
            for (i = 0; i < ARRAY_SIZE(ids); i++)
            {
               if (current_input->input_state(
                  ...
                  BIT256_SET_PTR(&loop_bits, ids[i][1]);
            }
         }
      }
      else
