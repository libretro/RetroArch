...
/* ===========================================================================
 * Threaded ALSA microphone ("alsathread"): a worker thread owns the
 * blocking snd_pcm_readi while the frontend reads from a fifo.
 *
 * There was a playback driver here too, and it was removed: the threaded
 * pipeline does the same decoupling a layer up, from the audio thread,
 * and does more of it - the resampler moves off the frame as well as the
 * blocking write. It could not even coexist with it, having no
 * wait_writable(), so selecting "alsathread" silently disabled the
 * pipeline and logged a fallback most people would never read. A core
 * that sets an audio callback is no exception: audio_driver_init_internal()
 * installs the audio thread wrapper for that case too, so the decoupling
 * is there without a second worker inside the driver.
 *
 * The capture side has no such replacement. There is no threaded
 * pipeline for microphones, and snd_pcm_readi blocks the caller, so this
 * worker is still the only thing between a core's mic read and the
 * device. It stays.
 *
 * Guarded by exactly the expression audio_driver.c uses to register
 * microphone_alsathread, so the vtable exists iff it is registered.
 * ======================================================================== */
