...
static void ps4_audio_mainloop(void *data)
{
   ...
   while (retro_atomic_load_acquire_int(&ps4->running))
   {
      ...
      cond                = RING_HELD(write_pos, play_pos, ps4->ring)
            < AUDIO_OUT_COUNT;

      if (!cond)
      {
         next_pos      += AUDIO_OUT_COUNT;
         if (next_pos  >= ps4->ring)
            next_pos    = 0;
      }
      else
      ...
      unsigned *new_rate)
{
   ...
   ps4->buffer_u32    = (uint32_t*)calloc(ps4->ring + AUDIO_OUT_COUNT,
         sizeof(uint32_t));
...
static size_t ps4_buffer_size(void *data)
{
   ...
   return ps4->ring * sizeof(uint32_t);
}
