 ...
 * ring's end. */
static unsigned ps4_ring_frames(unsigned rate, unsigned latency)
{
...
static void ps4_audio_mainloop(void *data)
{
   ...
   while (retro_atomic_load_acquire_int(&ps4->running))
   {
      ...
      cond                = RING_HELD(write_pos, read_pos, ps4->ring)
            < AUDIO_OUT_COUNT;

      if (!cond)
      {
         read_pos      += AUDIO_OUT_COUNT;
         if (read_pos  >= ps4->ring)
            read_pos    = 0;
      }
      else
      ...
      unsigned *new_rate)
{
   ...
   ps4->rate          = *new_rate ? *new_rate : rate;
   ps4->ring          = ps4_ring_frames(ps4->rate, latency);

   /* Cache aligned, not necessary but helpful. */
   ps4->buffer_u32    = (uint32_t*)calloc(ps4->ring + AUDIO_DEVICE_FRAMES,
         sizeof(uint32_t));
...
static ssize_t ps4_audio_write(void *data, const void *s, size_t len)
{
   ...
   if ((write_pos + sample_count) > ps4->ring)
   {
      memcpy(ps4->buffer_u32 + write_pos, s,
            (ps4->ring - write_pos) * sizeof(uint32_t));
      memcpy(ps4->buffer_u32, (uint32_t*)s +
            (ps4->ring - write_pos),
            (write_pos + sample_count - ps4->ring) * sizeof(uint32_t));
   }
   else
...
static size_t ps4_write_avail(void *data)
{
   ...
   _len = RING_FREE((uint16_t)
         ...
         retro_atomic_load_acquire_int(&ps4->read_pos), ps4->ring);
...
static size_t ps4_buffer_size(void *data)
{
   ...
   return (ps4->ring + AUDIO_DEVICE_FRAMES) * sizeof(uint32_t);
}
