...
#include "../drivers/gl_shaders/pipeline_fireworks.glsl.frag.h"
#endif
...
static void *gl_glsl_init(void *data, const char *path)
{
   ...
   *glsl->alias_define = '\0';
   ...
   shader_prog_info.vertex   = glsl_core ? stock_vertex_xmb_snow_modern : stock_vertex_xmb_snow_legacy;
#endif
   shader_prog_info.fragment = stock_fragment_xmb_fireworks;

   gl_glsl_compile_program(
         glsl,
         VIDEO_SHADER_MENU_7,
         &glsl->prg[VIDEO_SHADER_MENU_7],
         &shader_prog_info);
   gl_glsl_find_uniforms(glsl, 0, glsl->prg[VIDEO_SHADER_MENU_7].id,
         &glsl->uniforms[VIDEO_SHADER_MENU_7]);
