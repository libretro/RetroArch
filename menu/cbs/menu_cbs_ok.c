...
static enum msg_hash_enums action_ok_dl_to_enum(unsigned lbl)
{
   switch (lbl)
   {
     ...
     case ACTION_OK_DL_CHEEVOS_WEBHOOK_SETTINGS_LIST:
        return MENU_ENUM_LABEL_DEFERRED_CHEEVOS_WEBHOOK_SETTINGS_LIST;
...
STATIC_DEFAULT_ACTION_OK_FUNC(action_ok_cheevos_webhook_list, ACTION_OK_DL_CHEEVOS_WEBHOOK_SETTINGS_LIST)
...
static int action_ok_cheevos_webhook_start_association(const char *path,
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   if (command_event(CMD_EVENT_WEBHOOK_START_ASSOCIATION, NULL))
      return generic_action_ok_command(CMD_EVENT_RESUME);
