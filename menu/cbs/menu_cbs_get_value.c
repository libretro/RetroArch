...
static size_t menu_file_browser_stem_length(const char *path)
{
   const char *name = path_basename(path);
   const char *ext  = path_get_extension(path);

   /* A basename's sole leading dot is part of the filename. */
   return (*ext && ext > name + 1)
         ? (size_t)(ext - path - 1) : strlen(path);
}

static const char *menu_file_browser_get_display_extension(const char *path)
{
   size_t stem_len;
...
{
   const struct item_file *a = *(const struct item_file * const *)a_;
   const struct item_file *b = *(const struct item_file * const *)b_;
   size_t la = menu_file_browser_stem_length(a->path);
   size_t lb = menu_file_browser_stem_length(b->path);
   int rv    = strncasecmp(a->path, b->path, la < lb ? la : lb);
...
void menu_file_browser_prepare_extensions(file_list_t *list, unsigned mode)
{
   ...
   for (i = 0; i < list->size; i++)
      if (menu_file_browser_entry_is_file(list, i))
      {
         ...
         cbs->file_extension_state =
               mode == MENU_FILE_BROWSER_EXTENSION_DISPLAY_NEVER ? 1 : 0;
         count++;
      }
   ...
   /* Allocation failure leaves full filenames visible. */
   if (count > (size_t)-1 / sizeof(*files)
       || !(files = (const struct item_file**)malloc(count * sizeof(*files))))
      return;
   ...
   for (i = 0; i < list->size; i++)
      if (menu_file_browser_entry_is_file(list, i))
      {
         menu_file_list_cbs_t *cbs =
               (menu_file_list_cbs_t*)list->list[i].actiondata;
         cbs->file_extension_state = 1;
         files[count++] = &list->list[i];
      }

   /* Sort references only: the visible list retains its original order. */
   qsort(files, count, sizeof(*files), menu_file_browser_compare_stems);
   for (i = 1; i < count; i++)
      if (menu_file_browser_compare_stems(&files[i - 1], &files[i]) == 0)
      {
         ((menu_file_list_cbs_t*)files[i - 1]->actiondata)->file_extension_state = 2;
         ((menu_file_list_cbs_t*)files[i]->actiondata)->file_extension_state = 2;
      }
   free(files);
}
      ...
      unsigned state, char *display_name, size_t display_name_size)
{
   ...
   if (   !path || !*path || !display_name_size
       || !display_name || !state)
      return;

   strlcpy(display_name, path, display_name_size);

   extension = menu_file_browser_get_display_extension(path);
   if (extension)
   {
      size_t stem_len = (size_t)(extension - path - 1);
      if (stem_len < display_name_size)
         display_name[stem_len] = '\0';
   }

   if (state == 2)
   {
      strlcat(display_name, " (", display_name_size);
      strlcat(display_name, extension ? extension
