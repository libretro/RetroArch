#!/bin/sh
# Builds the menu packed-row settings harness.
#
# Like playlist_nav, this links the SHIPPING objects of a built
# RetroArch with only main() replaced - nothing is stubbed.  The rows
# under test are the menu settings that each own one half of a packed
# settings_t word, driven through their real action handlers, their
# display strings and the dropdown list the menu builds for them.
#
...
# the frontend is.
sed 's#-o obj-unix/release/retroarch\.o#-Dmain=rarch_harness_unused_main -o samples/settings/packed_rows/retroarch_nomain.o#' \
   "$cc_line" | sh

sed -e 's#-o obj-unix/release/retroarch\.o#-o samples/settings/packed_rows/harness_main.o#' \
    -e 's# retroarch\.c# samples/settings/packed_rows/packed_rows_test.c#' \
   "$cc_line" | sh
...
sed -e 's#obj-unix/release/retroarch\.o#samples/settings/packed_rows/retroarch_nomain.o samples/settings/packed_rows/harness_main.o#' \
    -e 's#-o retroarch #-o samples/settings/packed_rows/packed_rows_test #' \
   "$ld_line" | sh

echo "built samples/settings/packed_rows/packed_rows_test"
