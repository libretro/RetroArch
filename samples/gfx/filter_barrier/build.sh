#!/bin/sh
# Builds the softfilter barrier harness: the builtin filter set and the
# harness main, threads on, ASan. video_filter.c is NOT listed here - the
# harness includes it, so listing it too would define every symbol twice.
# SANITIZER= overrides the default (e.g. SANITIZER=thread).
set -e
cd "$(dirname "$0")"
R=../../..
FILTERS="$(ls $R/gfx/video_filters/*.c)"
SRCS="   $R/libretro-common/features/features_cpu.c \
...
gcc -O1 -g -fsanitize="$SAN" -std=gnu99 \
   -DRARCH_INTERNAL -DHAVE_THREADS -DHAVE_FILTERS_BUILTIN \
   -D_GNU_SOURCE \
   -I$R -I$R/libretro-common/include -I$R/deps \
   filter_barrier_test.c frontend_stubs.c $SRCS \
   -o filter_barrier_test -lpthread -lm
echo "built samples/gfx/filter_barrier/filter_barrier_test"
