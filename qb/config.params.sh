...
HAVE_C89_BUILD=no          # Enable a c89 build
HAVE_CXX_BUILD=no          # Enable a c++ build
...
C89_SSL=no                 # SSL/mbedtls is
HAVE_BUILTINMBEDTLS=auto   # Bake in the mbedtls library
C89_BUILTINMBEDTLS=no      # mbedtls is
CXX_BUILTINMBEDTLS=no      # mbedtls is
...
C89_NETWORKGAMEPAD=no      # Networked game pad is
...
C89_OPENGL_CORE=no         # OpenGL core is
...
C89_WAYLAND=no             # Wayland is
CXX_WAYLAND=no             # Wayland is
