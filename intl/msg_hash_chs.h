...
MSG_HASH(
   ...
   "屏幕的垂直刷新率。 用于计算合适的音频输入速率。 注意： 如果启用了“独立线程”，该项设置将被忽略。"
   )
MSG_HASH(
...
MSG_HASH(MENU_ENUM_SUBLABEL_VIDEO_FILTER,
      "加载一个由 CPU 控制的视频过滤器。注意：这可能显著降低游戏运行速度。某些视频过滤器仅对32位色（或16位色）核心生效。")
...
MSG_HASH(
      ...
      "只放大整数倍显示。基础分辨率与游戏、宽高比有关。如果「保持宽高比」选项未开启，不保证宽高放大倍数相同。"
      )
MSG_HASH(
