...
MSG_HASH(
   ...
   "ВНИМАНИЕ: на некоторых экранах при быстром мерцании крайне высок риск появления остаточного изображения. // Вставляет кадры чёрного цвета между обычными. Позволяет существенно уменьшить размытие в движении за счёт эмуляции развёртки ЭЛТ и ценой снижения яркости."
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "ВНИМАНИЕ: на некоторых экранах при быстром мерцании крайне высок риск появления остаточного изображения. // Имитирует простую бегущую строку развёртки на протяжении нескольких подкадров, разделяя экран по вертикали и отрисовывая каждую часть исходя из их количества."
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "Применяет программный графический фильтр. Возможно сильное снижение производительности. Некоторые фильтры работают только в ядрах с 16- или 32-битным цветом."
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "Вертикальная частота обновления экрана устройства. Используется для вычисления подходящей частоты дискретизации звука. Игнорируется при включении опции 'Рендеринг в отдельном потоке'."
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "Точная частота экрана (в Гц) для расчёта частоты звука: частота_звука = вход. частота игры * частота экрана / частота обнов. игры. Если ядро не сообщает значений, то используются станд. значения NTSC. Данный параметр должен быть как можно ближе к 60 Гц, чтобы избежать сильных откло[...]"
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "Срезать часть пикселей вокруг изображения, которые при разработке обычно остаются пустыми, но иногда могут содержать мусорные данные. Требуется перезапуск."
   )
...
MSG_HASH(
   ...
   "Включить режим вывода HDR, если поддерживается дисплеем. Примечание: scRGB может смягчать строгие маски CRT-шейдеров, т.к. после применения маски композитинг ОС делает преобразование в HDR10."
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "Использует настраиваемый интервал смены кадров VSync, понижая частоту обновления экрана на указанный коэффициент. Режим 'Авто' устанавливает значение на основе частоты обновления, сообщаемой ядром, улучшая подачу кадров в играх с 30 к/с на дисплеях 60 Гц или с 60 к/с на дисплеях [...]"
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "Динамическая подстройка действующей 'Задержки кадра' для снижения задержки ввода."
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "Замер времени кадра только в стабильном состоянии"
   )
MSG_HASH(
   MENU_ENUM_SUBLABEL_VIDEO_FRAME_TIME_SAMPLE_GATED,
   "Ограничивает расчёт частоты обновления экрана кадрами, когда контент работает стабильно (исключая меню, паузу, ускорение, и если время кадра в допустимом диапазоне). Показания диагностики становятся точнее ценой более медленной сходимости данных после загрузки контента."
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "Ограничение на количество записей в плейлисте 'Избранное'. При достижении предельного значения новые записи не будут добавляться до удаления старых. Значение '-1' отключает ограничение.\nВНИМАНИЕ: уменьшение значения удалит существующие записи!"
   )
MSG_HASH(
