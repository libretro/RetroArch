...
typedef struct microphone_driver
{
    ...
    *
    * @returns A handle to the general microphone driver context,
    ...
    *
    * Samples are provided in mono format,
    * in either signed 16-bit integer or 32-bit floating point.
    ...
    * unless set otherwise wioth set_nonblock_state().
    *
    ...
    * TODO mics are inactive when created
    * TODO intended for cores that provide a mic button
    * TODO paused driver won't resume, but will determine if mic resumes when unpaused
    * TODO active mics retrieve samples
    *
    * @param[in] driver_context Pointer to the driver context.
    * Will be the value that was returned by \c ::init().
