   ...
   if (type == UDEV_INPUT_MOUSE || type == UDEV_INPUT_TOUCHPAD )
   {
      ...
      if (has_absolutes)
      {
          ...
          if (ioctl(fd, EVIOCGABS(ABS_X), &absinfo) != -1)
          {
             ...
             if (absinfo.minimum >= absinfo.maximum )   RARCH_LOG("[udev]: ABS_X MIN>=MAX min=%d , max=%d\n", absinfo.minimum, absinfo.maximum );
          ...
          {
             /* This shouldnt happen log if it does */
             if (absinfo.minimum >= absinfo.maximum )   RARCH_LOG("[udev]: ABS_Y MIN>=MAX min=%d , max=%d\n",  absinfo.minimum, absinfo.maximum );
