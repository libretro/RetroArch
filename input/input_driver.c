      ...
      size_t word_len)
{
   ...
   if (string_is_equal(word, "\xe2\x87\xa7"))      /* shift(up arrow) 1,3,5 */
      *osk_idx = (*osk_idx) | 0x01 ;
   else if (string_is_equal(word, "\xe2\x87\xa9")) /* shift(down arrow) 0,2,4 */
...
bool input_read_keyboard()
{
      ...
      char app[256]       = {0};
      intfstream_t *file;
      fill_pathname_application_dir(app, sizeof(app));
      fill_pathname_resolve_relative(path, app,"keyboard.json", sizeof(path));
