...
S_UINT_EX(video_sdl_display_server, VIDEO_SDL_DISPLAY_SERVER,
      ...
      "Let the SDL window switch among the display modes it lists. 'Auto' uses it only when the native display server cannot switch modes; 'Always' overrides the native server, which disables custom CRT timings.")
