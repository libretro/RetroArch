...
desktop_menu_dock_screenshot = "right,1,0,0,-,0"
...
# Every dock has a row: six fields docked, eight (with x,y) floating.
for d in search playlists core boxart title screenshot logo core_info log; do
   ...
   case "$r" in
      float,*) [ "$n" -eq 8 ] || fail "floating dock $d row has $n fields: '$r'" ;;
      left,*|right,*|top,*|bottom,*)
         [ "$n" -eq 6 ] || fail "dock $d row has $n fields: '$r'" ;;
...
case "$(row desktop_menu_dock_core_info)" in
   float,1,300,200,-,0,100,60) ;;
   *) fail "floating dock not restored in place: '$(row desktop_menu_dock_core_info)'" ;;
esac
# The remaining thumbnail docks are still tabbed onto boxart.
for d in title logo; do
