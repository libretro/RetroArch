...
static void wg_end_http_request
(
  webhook_game_async_io_request_t* request
)
{
...
static void wg_handle_http_callback
(
  retro_task_t* task,
  void* task_data,
  void* user_data,
...
{
  struct webhook_game_async_io_request_t *request = (struct webhook_game_async_io_request_t*)user_data;
  ...
  wg_end_http_request(request);
}

static void wg_begin_http_request
(
  webhook_game_async_io_request_t* request
)
{
  ...
  (
    ...
    "POST",
    request->headers,
    wg_handle_http_callback,
    request
  );
}

static void wg_prepare_http_request
(
  webhook_game_async_io_request_t* request,
  int game_id,
...
)
{
  ...
  rc_url_builder_append_unum_param(&builder, "g", game_id);
  rc_url_builder_append_unum_param(&builder, "e", game_event);
  request->request.post_data = rc_url_builder_finalize(&builder);
}

void wg_send_game_async
(
  webhook_game_async_io_request_t* request,
  int game_id,
  game_event_t game_event
...
{
  wg_prepare_http_request(request, game_id, game_event);

  wg_begin_http_request(request);
}

void wg_update_game(int game_id, game_event_t game_event)
{
  webhook_game_async_io_request_t *request = (webhook_game_async_io_request_t*) calloc(1, sizeof(webhook_game_async_io_request_t));
  ...
  wg_send_game_async(request, game_id, game_event);
}
